			...
			<td class="" ng-if="permission_check.see_module_manage_system_install">
				...
				<div ng-if="do == 'recycle'" class="link-group">
					...
					<?php  if($type == MODULE_RECYCLE_INSTALL_DISABLED) { ?>
						...
						<?php  } ?>
					<?php  } else { ?>
						<a ng-if="module.cloud_id > 0 && permission_check.see_module_manage_system_shopinfo" ng-href="{{ 'https://xydai.cn/module-' + module.cloud_id + '.html' }}" target="_blank" >查看商城详情</a>
					<?php  } ?>
				</div>
				<!-- recycle end -->
...
<div class="js-system-module-detail" ng-controller="detailCtrl" ng-cloak>
	...
	<div class="user-head-info ">
		...
		<a ng-if="isFounder == 1 && moduleinfo.cloud_mid != ''" ng-href="{{ 'https://xydai.cn/module-' + moduleinfo.cloud_mid + '.html' }}" target="_blank" class="btn btn-primary ">查看商城详情</a>
		<?php  } ?>
	</div>
	...
	<table class="table we7-table table-hover vertical-middle" ng-show="show == 'upgrade'" ng-if="isFounder == 1 && upgradeInfo.from == 'cloud' && branch.displayorder >= upgradeInfo.site_branch.displayorder" ng-repeat="branch in upgradeInfo.branches">
		...
		<tr>
			...
			<td colspan="4" class="text-right">
				...
				<a href="https://xydai.cn/module-{{upgradeInfo.id}}.html" ng-if="branch.displayorder > upgradeInfo.site_branch.displayorder || (branch.displayorder == upgradeInfo.site_branch.displayorder && branch.id > upgradeInfo.site_branch.id)" class="color-default view-detail" target="_blank">查看分支详情</a>
			</td>
		</tr>
